...
import { ThumbsUp, ThumbsDown, Copy, Check, ChevronDown, ChevronUp, Bot, User } from 'lucide-react';
...
export const PlanView = ({ messages, onFeedback }: PlanViewProps) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto px-2 sm:px-6 py-6 space-y-6 scrollbar-hide">
        ...
        {messages.map((msg, i) => {
          ...
          return (
            ...
            >
              {/* AI Avatar */}
              {!isUser && (
                <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-primary/20 to-accent/20 flex items-center justify-center shrink-0 mt-1 ring-1 ring-primary/10">
                  <BrandLogo size={18} />
                </div>
              )}
              ...
              <div className={`flex flex-col ${isUser ? 'items-end max-w-[80%]' : 'items-start flex-1 min-w-0'}`}>
                {/* Role label */}
                <span className={`text-[10px] font-medium mb-1 px-1 ${isUser ? 'text-muted-foreground' : 'text-primary'}`}>
                  {isUser ? 'আপনি' : 'TIVO AI'}
                </span>
                ...
                <div
                  className={`rounded-2xl text-sm leading-relaxed ${
                    isUser
                      ? 'bg-primary/10 border border-primary/20 text-foreground px-4 py-3'
                      : 'bg-muted/30 border border-border/50 text-foreground px-5 py-4 w-full'
                  }`}
                >
                  {isUser ? (
                    <div>
                      <p className={`whitespace-pre-wrap break-words text-[15px] ${isLongUserMsg && !isExpanded ? 'line-clamp-4' : ''}`}>
                        {msg.content}
                      </p>
                      {isLongUserMsg && (
                        <button
                              ...
                              <div className="relative group my-3">
                                <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                  <button
                                    ...
                                    className="p-1.5 rounded-lg bg-muted/50 hover:bg-muted text-muted-foreground hover:text-foreground text-xs"
                                  >
              ...
              {/* User Avatar */}
              {isUser && (
                <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-accent/20 to-primary/20 flex items-center justify-center shrink-0 mt-1 ring-1 ring-accent/10">
